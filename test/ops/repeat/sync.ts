...
import {pipeSync, repeat} from '../../../src';

export default () => {
    it('must not copy for 0 or negative count', () => {
        const i1 = pipeSync([1, 2], repeat(0));
        expect([...i1]).to.eql([1, 2]);
        const i2 = pipeSync([1, 2], repeat(-1));
        expect([...i2]).to.eql([1, 2]);
    });
    it('must copy for positive numbers', () => {
        const i1 = pipeSync([1, 2], repeat(1));
        expect([...i1]).to.eql([1, 1, 2, 2]);
        const i2 = pipeSync([1, 2], repeat(2));
