...
import {pipe, map, delay, waitRace} from '../../../src';
...
export default () => {
    ...
    it('must resolve all simple values', async () => {
        const i = pipe(_async([1, 2, 3, 4]), waitRace(2));
        expect(await _asyncValues(i)).to.have.members([1, 2, 3, 4]);
    });
    ...
    it('must handle invalid size of cache', async () => {
        const i = pipe(_async([1, 2, Promise.resolve(3), 4]), waitRace(-1));
        expect(await _asyncValues(i)).to.have.members([1, 2, 3, 4]);
    });
    it('must reject when a value rejects', async () => {
        const i = pipe(_async([1, Promise.reject(2) as any, 3]), waitRace(2));
        let err;
        try {
