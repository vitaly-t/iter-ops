...
export default () => {
    ...
    it('must flatten arrays', async () => {
        const output = pipeAsync(_async([[1, 2], _async([3, 4])]), flat());
        expect(await _asyncValues(output)).to.eql([1, 2, 3, 4]);
    });
    it('must flatten mixed iterables', async () => {
        const output1 = pipeAsync(_async<any>([[1, 2], 3, [4, 5]]), flat());
        ...
        const output2 = pipeAsync(input, flat(2));
        const output3 = pipeAsync(input, flat(3));
    ...
    it('must handle empty iterables', async () => {
        const output1 = pipeAsync(_async([]), flat());
        const output2 = pipeAsync(_async([[]]), flat());
        const output3 = pipeAsync(_async([[], []]), flat());
        const output4 = pipeAsync(_async(['']), flat());
    ...
    it('must find values after empty', async () => {
        const output = pipeAsync(_async([[], [1]]), flat());
