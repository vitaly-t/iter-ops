    ...
    describe('with option', () => {
        describe('carry', () => {
            it('must be able to carry back', async () => {
                const i = pipe(_async([0, 1, 2, 0, 0, 3, 4, 0, 0]), split(a => a === 0, {
                    carryStart: -1,
                    carryEnd: -1
                }));
                expect(await _asyncValues(i)).to.eql([[0], [1, 2, 0], [0], [3, 4, 0], [0], []]);
            });
            it('must be able to carry forward', async () => {
                const i = pipe(_async([0, 1, 2, 0, 0, 3, 4, 0, 0]), split(a => a === 0, {carryStart: 1, carryEnd: 1}));
                expect(await _asyncValues(i)).to.eql([[], [0, 1, 2], [0], [0, 3, 4], [0], [0]]);
            });
        });
        describe('toggle', () => {
            describe('without carrying', () => {
                it('must handle any regular scenario', async () => {
                    const i1 = pipe(_async([0, 1, 2, 0, 0, 3, 4]), split(a => !a, {
                        toggle: true,
                        carryStart: 'bla' as any, // for coverage
                        carryEnd: 'bla' as any // for coverage
                    }));
                    expect(await _asyncValues(i1)).to.eql([[1, 2], [3, 4]]);

                    const i2 = pipe(_async([1, 2, 0, 0, 3, 4, 0]), split(a => !a, {
                        toggle: true,
                        carryStart: SplitValueCarry.none // just for coverage
                    }));
                    expect(await _asyncValues(i2)).to.eql([[], []]);

                    const i3 = pipe(_async([1, 2, 0, 3, 4, 0]), split(a => !a, {toggle: true}));
            ...
            describe('with carrying', () => {
                it('must work with carrying back', async () => {
                    ...
                    expect(await _asyncValues(i2)).to.eql([[1, 2], [3, 4], [5]]);
                });
                it('must work with carrying forward', async () => {
                    const i1 = pipe(_async([1, 2, 3, 4, 5]), split(a => !!a, {toggle: true, carryStart: 1}));
                    expect(await _asyncValues(i1)).to.eql([[1], [3], [5]]);

                    const i2 = pipe(_async([0, 1, 2, 0, 3, 4, 0, 5]), split(a => a === 0, {
                        toggle: true,
                        carryStart: 1,
                        carryEnd: 1
                    }));
                    expect(await _asyncValues(i2)).to.eql([[0, 1, 2], [0, 0, 5]]);
                });
                it('must handle triggers without values', async () => {
                    const i1 = pipe(_async([0, 0, 0, 0]), split(a => a === 0, {
                        toggle: true,
                        carryStart: 1,
                        carryEnd: 1
                    }));
                    ...
                    const i2 = pipe(_async([0, 0, 0, 0]), split(a => a === 0, {
                        toggle: true,
                        carryStart: -1,
                        carryEnd: -1
                    }));
                    expect(await _asyncValues(i2)).to.eql([[0, 0], [0, 0]]);
                });
            });
        });
    });
    describe('indexes', () => {
        describe('for split, no carrying', () => {
            it('must report correct indexes', async () => {
                const indexes: ISplitIndex[] = [];
                const i = pipe(_async('one two'), split((a, idx) => {
                    indexes.push(idx);
                    return a === ' ';
                }));
        ...
        describe('for toggle', () => {
            ...
            it('must report correct indexes for carry=back', async () => {
                const indexes: ISplitIndex[] = [];
                const i = pipe(_async([1, 2, 3, 4, 5]), split((a, idx) => {
                    indexes.push(idx);
                    return true;
                }, {toggle: true, carryStart: -1, carryEnd: -1}));
                await _asyncValues(i);
